...
class Network:
    def main(self, inputs):
        ...
        l5 = multy_layer_2(l4, 16 * size // 2)

        l6 = deconv_layer(l5, 8 * size // 2)
        l6 = concatenate([l4, l6], axis=3)
        l6 = multy_layer(l6, 8 * size)

        l7 = deconv_layer(l6, 4 * size // 2)
        l7 = concatenate([l3, l7], axis=3)
        l7 = multy_layer(l7, 4 * size)

        l8 = deconv_layer(l7, 2 * size // 2)
        l8 = concatenate([l2, l8], axis=3)
        l8 = multy_layer(l8, 2 * size)

        l9 = deconv_layer(l8, size // 2)
        l9 = concatenate([l1, l9], axis=3)
        l9 = multy_layer(l9, size)
        l9 = Conv2D(size // 2, kernel_size=3, padding='same')(l9)
        l9 = LeakyReLU()(l9)

        output = Conv2D(1, 2, padding='same')(l9)
        return output
